...
plt_province_stats <- function(df,is_confirmed=FALSE) {
    ...
    if (!is_confirmed) {
        for (province in provinces) {
            fname <- gsub(" ","_",sprintf("plts/canada/K19_%s_Outcomes.png",province))
        ...
        for (province in provinces) {
            fname <- gsub(" ","_",sprintf("plts/canada/K19_%s_Confirmed.png",province))
            png(fname, width=700, height=432)
            print(fname)
            pstats <- df[(df$province_state==province),]
            pstats <- pstats[(pstats$date > "03-12-2020"),]
...
plt_state_stats <- function(df,is_confirmed=FALSE) {
    ...
    if (!is_confirmed) {
        for (state in states) {
            ...
            pstats <- accumulate(pstats,'confirmed')
            pstats <- accumulate(pstats,'recovered')
            pstats <- accumulate(pstats,'deaths')
            #pstats <- pstats[(pstats$date > "03-12-2020"),]
            plot(as.Date(pstats$date,"%m-%d-%Y"),pstats$confirmed,type='b',xaxs='i',panel.first=grid(),
                 xlab="Days",
                 ylab=paste("Outcomes (Log Scale)"),
            ...
            legend("topleft",
                 legend=c(
                   sprintf('Confirmed (%s)',formatC(max(pstats$confirmed),format="d",big.mark=",")),
                   sprintf('Recovered (%s)',formatC(max(pstats$recovered),format="d",big.mark=",")),
                   sprintf('Deaths (%s)',formatC(max(pstats$deaths),format="d",big.mark=","))
                 ),
                 cex=.7,
                 pch=20,
                 col=c('black','blue','red'),
        ...
        for (state in states) {
            ...
            legend("topleft",
                 legend=sprintf('Confirmed (%s)',formatC(max(pstats$confirmed),format="d",big.mark=",")),
                 cex=.7,
                 pch=20,
                 col='black',
                 lty=1
