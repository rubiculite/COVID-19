...
plt_province_stats <- function(df) {
  ...
  provinces <- c('Newfoundland','Nova Scotia','Prince Edward Island','New Brunswick','Quebec','Ontario','Manitoba','British Columbia')
  for (province in provinces) {
    png(gsub(" ","_",sprintf("plts/K19_%s_Confirmed.png",province)), width=700, height=432)
    print(sprintf("plts/K19_%s_Confirmed.png",province))
...
plt_prov_stats <- function(df) {
  ...
  first <- TRUE
  png("plts/K19_Major_Canada_Province_Stats.png", width=700, height=432)
  for (province in datum$province) {
    if (first) {
      plot(
...
plt_canada_stats <- function(df) {
  ...
  png("plts/K19_Canada_Stats.png", width=700, height=432)
